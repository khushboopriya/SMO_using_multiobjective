...
//-------------------- Structures
struct quantum   // this is required to break the particles(group)
{
	double q[DMax];
	int size; 
};

struct SS 		// this is the structure for search space
{ 
	int D;			// dimensions
	...
	double maxS[DMax];	// Search space max
	double minS[DMax];	//search space min
	int valueNb; 		 	// if >0, the search space is a list of "valueNb"values.
};

struct param 		//this represents the parameters of swarm
{

	double w; 			// Inertial weight
	double c1;			// Confidence coefficient(max)
	double c2;			// confidence coefficient (min)
...
struct archive {				//this only memorizes the previous positions
...
void memSave(struct position P);  //positions are saved in this function
